...
function renderLogo() {
    ...
    const hasCustomLogo = db.settings.logoUrl && db.settings.logoUrl.trim() !== '';
...
function openEditLogoModal() {
    document.getElementById('edit-logo-url').value = db.settings.logoUrl || '';
...
function renderFilosofi() {
    ...
    const photos = db.settings.filosofiPhotos || [];
...
function updateNavbarState() {
    ...
    } else if (currentUserKey && db.members[currentUserKey]) {
...
function renderMembers() {
    ...
    let i = 0;
    for (const [key, data] of Object.entries(db.members)) {
        const theme = themeStyles[i % themeStyles.length];
        const shortName = data.name.split(' ').slice(0, 2).join(' ');
        const initials = getInitials(data.name);

        let avatarContentHTML = '';
        if (data.avatarUrl && data.avatarUrl.trim() !== '') {
            avatarContentHTML = `<img src="${formatDriveImageUrl(data.avatarUrl)}" alt="${data.name}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">`;
        } else {
            avatarContentHTML = `<div class="w-full h-full bg-sand/30 dark:bg-darkInput text-terracotta font-serif font-bold text-xl sm:text-2xl flex items-center justify-center">${initials}</div>`;
        }

        let socialBadgesHTML = '';
        if (data.ig && data.ig.trim() !== '') {
            socialBadgesHTML += `<span class="text-pink-600 dark:text-pink-300 text-[10px] sm:text-[11px] font-semibold bg-pink-50 dark:bg-pink-900/40 px-2 py-0.5 sm:py-1 rounded-full border border-pink-100 dark:border-pink-800 flex items-center gap-1"><i class="fab fa-instagram"></i>@${data.ig}</span>`;
        }
        if (data.tiktok && data.tiktok.trim() !== '') {
            socialBadgesHTML += `<span class="text-gray-800 dark:text-gray-300 text-[10px] sm:text-[11px] font-semibold bg-gray-100 dark:bg-gray-800 px-2 py-0.5 sm:py-1 rounded-full border border-gray-300 dark:border-gray-600 flex items-center gap-1"><i class="fab fa-tiktok"></i>@${data.tiktok}</span>`;
        }

        let adminDeleteBtnHTML = '';
        if (currentUserKey === 'admin') {
            adminDeleteBtnHTML = `
                <button onclick="deleteMember('${key}', event)" class="absolute top-2.5 right-2.5 bg-red-600 hover:bg-red-700 text-white w-7 h-7 rounded-full flex items-center justify-center shadow-md transition-transform hover:scale-110 z-20" title="Hapus Anggota (Admin Only)">
                    <i class="fas fa-trash-alt text-xs"></i>
                </button>
            `;
        }
        ...
        const html = `
            <div class="relative group">
                ${adminDeleteBtnHTML}
                <a href="#member-${key}" class="block bg-white dark:bg-darkCard rounded-2xl p-4 sm:p-6 shadow-md hover:shadow-xl transition-all duration-300 border-b-4 ${theme.border} group cursor-pointer text-center transform hover:-translate-y-1 sm:hover:-translate-y-2">
                    <div class="relative w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 mx-auto mb-3.5 sm:mb-5 overflow-hidden rounded-full border-3 sm:border-4 border-cream dark:border-gray-700 shadow-sm">
                        ${avatarContentHTML}
                    </div>
                    <h3 class="text-base sm:text-lg md:text-xl font-serif font-bold text-coffee dark:text-cream truncate mb-1.5 sm:mb-2" title="${data.name}">${shortName}</h3>
                    <span class="inline-block px-3 py-1 rounded-full text-[9px] sm:text-[10px] font-bold ${theme.badgeBg} ${theme.badgeText} uppercase tracking-widest mb-2 sm:mb-3 shadow-sm">${data.role}</span>
                    <p class="text-coffee/70 dark:text-gray-400 text-xs sm:text-sm italic line-clamp-2 mt-1">"${data.quote.replace(/"/g, '')}"</p>
                    <div class="mt-2.5 sm:mt-3 flex flex-wrap justify-center gap-1.5 sm:gap-2">
                        ${socialBadgesHTML}
                    </div>
                </a>
            </div>
...
function loadMemberData(memberKey) {
    ...
    if (currentUserKey === 'admin') {
        editRoleBtnHTML = `
            <button onclick="openEditRoleModal()" class="ml-2 text-terracotta hover:text-coffee dark:hover:text-cream transition-colors text-xs sm:text-sm flex-shrink-0" title="Edit Julukan (Admin Only)">
...
function openEditProfileModal() {
    const data = db.members[activeMemberKey]; 
    document.getElementById('edit-prof-avatar-url').value = data.avatarUrl || '';
    document.getElementById('edit-prof-quote').value = data.quote.replace(/"/g, '');
...
function saveProfileData() {
    ...
    db.members[activeMemberKey].avatarUrl = avatarUrl;
    db.members[activeMemberKey].quote = `"${quote}"`;
    db.members[activeMemberKey].ig = ig.replace('@', '');
...
function renderGallery() {
    ...
    const albumKeys = Object.keys(db.albums);
    if (albumKeys.length === 0) {
        ...
        for (const [key, data] of Object.entries(db.albums)) {
            ...
            if (currentUserKey === 'admin') {
                deleteAlbumBtnHTML = `
                    <button onclick="deleteAlbum('${key}', event)" class="absolute top-2.5 right-2.5 bg-red-600 hover:bg-red-700 text-white w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center shadow-lg transition-transform hover:scale-110 z-20" title="Hapus Album (Admin Only)">
                        <i class="fas fa-trash-alt text-xs"></i>
                    </button>
                `;
            }
            ...
            const html = `
                <div class="relative group">
                    ${deleteAlbumBtnHTML}
                    <a href="#album-${key}" class="gallery-item polaroid bg-white dark:bg-darkCard block w-full cursor-pointer rounded-sm sm:rounded-md">
                        <img src="${coverUrl}" alt="${data.title}" class="w-full h-52 sm:h-60 md:h-64 object-cover mb-3 sm:mb-4 rounded-sm">
                        <p class="font-serif text-lg sm:text-xl font-bold text-coffee dark:text-cream truncate">${data.title}</p>
                        <p class="text-xs sm:text-sm text-coffee/60 dark:text-gray-400 line-clamp-1 mt-0.5">${data.desc}</p>
                    </a>
                </div>
            `;
            container.innerHTML += html;
...
function loadAlbumData(albumKey) {
    ...
    document.getElementById('album-desc').textContent = data.desc;
    ...
    if (data.photos.length === 0) {
        ...
        data.photos.forEach((photoUrl, pIndex) => {
            ...
            const imgHTML = `
                <div class="relative overflow-hidden rounded-xl shadow-sm sm:shadow-md hover:shadow-xl transition-shadow duration-300 group">
                    ${deleteBtnHTML}
...
function renderVideos() {
    ...
    if (db.videos.length === 0) {
        ...
        db.videos.forEach((vid, index) => {
            ...
            container.innerHTML += `
                <div class="bg-white dark:bg-darkCard rounded-xl overflow-hidden shadow-md p-3.5 sm:p-4 border border-sand dark:border-gray-700">
                    ${playerHTML}
...
function saveNewMember() {
    ...
    if (db.members[id]) return alert('ID Anggota sudah terpakai!');

    db.members[id] = {
        name: name, role: role || 'Anggota', dob: dob || '-', quote: quote ? `"${quote}"` : '"Halo semuanya!"',
...
function showPage(pageId) {
    ...
    document.querySelectorAll('nav .nav-link, #mobile-menu a').forEach(link => {
        if (link.getAttribute('href') === '#' + navToHighlight) {
            link.classList.add('text-terracotta', 'dark:text-terracotta');
            link.classList.remove('text-coffee', 'dark:text-gray-300');
        } else {
            link.classList.add('text-coffee', 'dark:text-gray-300');
            link.classList.remove('text-terracotta', 'dark:text-terracotta');
        }
    });
